...
function Wheel() {
  ...
  const rotateWheel = () => {
    ...
    if(currentRotation === totalItems || (currentBatch.batch.length * (currentBatch.position + 1) === currentRotation)) {
      // console.log("Im triggering", currentRotation === totalItems, currentBatch.batch.length * (currentBatch.position + 1) === currentRotation)
      const newPosition = currentRotation === totalItems ? 0 : currentBatch.position + 1;
      ...
      if(newPosition + 1 === totalFullRotations && lastBatchLength !== 4) {
        ...
        batch = items.slice(newPosition * 4).concat(lastPrevItems);
          ...
          batch = items.slice(newPosition * 4, (newPosition * 4) + 3).concat(lastPrevItem)
        }
...
const Container = styled.ul`
  ...
  height: 43vw;
  position: absolute;
  transform: translateX(65%) rotate(${props => props.rotateDegress}deg);
  cursor: pointer;
